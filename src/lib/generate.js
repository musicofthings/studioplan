...
export async function genBlueprint(ask, f) {
  const prompt =
    ...
    `}`;
  return normalizeBlueprint(parseJSON(await ask(prompt, STRATEGIST, 4096)));
}
...
export async function genIdeas(ask, niche, count) {
  ...
  return normalizeIdeas(parseJSON(await ask(prompt, STRATEGIST, 2000)));
}
...
export async function genToolkit(ask, niche) {
  ...
  return normalizeToolkit(parseJSON(await ask(prompt, STRATEGIST, 2000)));
}
