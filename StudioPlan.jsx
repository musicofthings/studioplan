...
async function genBlueprint(f) {
  const prompt =
    ...
    `}`;
  return parseJSON(await callClaude(prompt, STRATEGIST));
}

async function genShort(f) {
  const prompt =
    ctx(f) +
  ...
  return callClaude(prompt, STRATEGIST);
}

async function genLong(f, chapters) {
...
async function genIdeas(niche, count) {
  const prompt =
...
async function genToolkit(niche) {
  const prompt =
...
export default function StudioPlan() {
  ...
  async function runIdeas() {
    ...
    try {
      const list = await genIdeas(ideas.niche, ideas.count);
  ...
  async function runKit() {
    ...
    try {
      const data = await genToolkit(kit.niche);
...
#hashtag #hashtag #hashtag`;
  ...
  return (
    <div className="sp-root min-h-screen bg-[#f6f5f2] text-[#17171c]">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bricolage+Grotesque:opsz,wght@12..96,500..800&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
        .sp-root{font-family:'Inter',system-ui,-apple-system,sans-serif;}
        .sp-display{font-family:'Bricolage Grotesque','Inter',sans-serif;letter-spacing:-0.01em;}
        .sp-mono{font-family:'JetBrains Mono',ui-monospace,monospace;}
        .sp-root *{scrollbar-width:thin;scrollbar-color:#d9d8d2 transparent;}
      `}</style>
      ...
      <div className="mx-auto flex min-h-screen max-w-[1240px]">
        ...
        <aside className="sticky top-0 hidden h-screen w-56 shrink-0 flex-col border-r border-[#e7e6e1] bg-white px-3 py-5 md:flex">
          ...
          <div className="mt-auto rounded-xl bg-[#f4f4f2] p-3">
            <p className="text-[11px] leading-relaxed text-[#8b8b95]">
              Scripts and packaging are generated live by Claude. Everything you make is saved to your
              masterplan for this session.
            </p>
          </div>
        </aside>
